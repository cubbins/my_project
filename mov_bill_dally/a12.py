...
def analyze_trend(bars):
    ...
    for i, bar in enumerate(bars):
        ...
        # --- NEW: switch-difference calculation ---
        if switch_price is None:
            switch_diff = None
        else:
            switch_diff = bar.price - switch_price
...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------
    # NEW: Build a large 2D array of all variables
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Printing with blank lines on transitions
    # --------------------------------------------------------
    last_state = None
    ...
    for r in results:
        ...
        print(f"    switch_price={r['switch_price']}  switch_diff={r['switch_diff']}")

        last_state = r["state"]

    # --------------------------------------------------------
    # Print summary table from the new 2D array
    # --------------------------------------------------------
    print("\n=== SUMMARY TABLE (seq, date, price, idx, state, switch_price, switch_diff) ===")

    for r in results:
        print(
            f"{r['seq']:5d}  {r['date']}  {r['price']:10.4f}  {r['idx']:3d}  "
            f"{r['state']:6s}  {r['switch_price']}  {r['switch_diff']}"
        )
