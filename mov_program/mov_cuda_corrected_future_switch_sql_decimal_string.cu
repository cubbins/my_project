...
static int display_precision_for_value(double x)
{
    double ax = std::abs(x);
    if (ax > 0.0 && ax < 0.01) return 10;
    if (ax > 0.0 && ax < 1.0)  return 8;
    return 4;
}

static int display_precision_for_series(const std::vector<double>& values)
{
    double min_abs = std::numeric_limits<double>::infinity();
    for (double v : values) {
        double av = std::abs(v);
        if (av > 0.0) min_abs = std::min(min_abs, av);
    }
    if (!std::isfinite(min_abs)) return 4;
    if (min_abs < 0.01) return 10;
    if (min_abs < 1.0)  return 8;
    return 4;
}

static double future_switch_precision_for_last_price(double last_price)
{
    // Absolute 0.1 precision is far too coarse for assets such as FX rates
    // near 0.006. Use scale-relative precision with a small lower bound.
    double ax = std::abs(last_price);
    if (ax <= 0.0 || !std::isfinite(ax)) return 1e-10;
    return std::max(ax * 1.0e-6, 1.0e-12);
}

static std::string fmt_double_scaled(double v)
{
    std::ostringstream oss;
    oss << std::fixed << std::setprecision(display_precision_for_value(v)) << v;
    return oss.str();
}

static std::string sql_decimal_string(double v)
{
    // Preserve small prices/FX rates when sending to SQL Server.
    // Binding C++ double directly as SQL_DECIMAL can cause driver-side
    // scale rounding. Sending a fixed decimal string lets SQL Server
    // convert into DECIMAL(38,18) accurately.
    std::ostringstream oss;
    oss << std::fixed << std::setprecision(18) << v;
    return oss.str();
}

static std::string fmt_opt_double(const std::optional<double>& v, int prec = -1)
{
    if (!v.has_value()) {
        return "None";
    }
    std::ostringstream oss;
    int use_prec = (prec >= 0) ? prec : display_precision_for_value(*v);
    oss << std::fixed << std::setprecision(use_prec) << *v;
    return oss.str();
}
    ...
    double precision = -1.0)
{
    ...
    if (precision <= 0.0) {
        precision = future_switch_precision_for_last_price(last_price);
    }
    if (last_price <= 0.0) {
    ...
    int tail_count = 12)
{
    ...
    for (int i = start; i < M; ++i) {
        std::cout << std::setw(4) << i << "  "
                  ...
                  << std::fixed << std::setprecision(display_precision_for_series(input_clos))
    ...
    std::cout << "\nOriginal final input value = " << std::fixed << std::setprecision(display_precision_for_series(input_clos))
              << input_clos[M - 1] << "\n";
    std::cout << "GPU final working value    = " << std::fixed << std::setprecision(display_precision_for_series(input_clos))
              << gpu.clos[M - 1] << "\n";
    std::cout << "CPU final working value    = " << std::fixed << std::setprecision(display_precision_for_series(input_clos))
              << cpu.clos[M - 1] << "\n";
}
...
static void print_trend_reports(const std::vector<TrendRow>& results)
{
    ...
    for (const auto& r : results) {
        ...
        std::cout
            << std::setw(5) << r.seq << "  "
            << r.date << "  "
            << std::setw(14) << std::fixed << std::setprecision(display_precision_for_value(r.price)) << r.price << "  "
            << std::setw(3) << r.idx << "  "
            << std::setw(6) << r.state << "  "
            << std::setw(8) << wall_val << "  "
            << std::setw(8) << d_val << "  "
    ...
    const std::vector<TrendRow>& rows)
{
    ...
    SQLINTEGER p_wall = 0;
    SQLINTEGER p_d = 0;
    char       p_switch_price[80] = {0};
    char       p_switch_diff[80] = {0};

    SQLLEN ind_asset = SQL_NTS;
    SQLLEN ind_seq_no = 0;
    SQLLEN ind_trade_date = SQL_NTS;
    SQLLEN ind_close_price = SQL_NTS;
    ...
    rc = SQLBindParameter(insertStmt, col++, SQL_PARAM_INPUT, SQL_C_CHAR,   SQL_VARCHAR, 79, 0, p_close_price,   sizeof(p_close_price), &ind_close_price);
    ...
    rc = SQLBindParameter(insertStmt, col++, SQL_PARAM_INPUT, SQL_C_CHAR,   SQL_VARCHAR, 79, 0, p_lph,           sizeof(p_lph), &ind_lph);
    ...
    rc = SQLBindParameter(insertStmt, col++, SQL_PARAM_INPUT, SQL_C_CHAR,   SQL_VARCHAR, 79, 0, p_ldl,           sizeof(p_ldl), &ind_ldl);
    ...
    rc = SQLBindParameter(insertStmt, col++, SQL_PARAM_INPUT, SQL_C_CHAR,   SQL_VARCHAR, 79, 0, p_switch_price,  sizeof(p_switch_price), &ind_switch_price);
    CHECK_ODBC_RC(rc, SQL_HANDLE_STMT, insertStmt, "Bind insert switch_price");
    rc = SQLBindParameter(insertStmt, col++, SQL_PARAM_INPUT, SQL_C_CHAR,   SQL_VARCHAR, 79, 0, p_switch_diff,   sizeof(p_switch_diff), &ind_switch_diff);
    ...
    for (size_t i = 0; i < rows.size(); ++i) {
        ...
        std::snprintf(p_close_price, sizeof(p_close_price), "%s", sql_decimal_string(clos[i]).c_str());
        ind_close_price = SQL_NTS;
        p_savemov = savemov[i];
        std::snprintf(p_state, sizeof(p_state), "%s", r.state.c_str());

        if (r.chg)          { p_chg = *r.chg; ind_chg = 0; } else ind_chg = SQL_NULL_DATA;
        if (r.lph_price)    { std::snprintf(p_lph, sizeof(p_lph), "%s", sql_decimal_string(*r.lph_price).c_str()); ind_lph = SQL_NTS; } else ind_lph = SQL_NULL_DATA;
        if (r.lph_seq)      { p_lph_seq = *r.lph_seq; ind_lph_seq = 0; } else ind_lph_seq = SQL_NULL_DATA;
        if (r.ldl_price)    { std::snprintf(p_ldl, sizeof(p_ldl), "%s", sql_decimal_string(*r.ldl_price).c_str()); ind_ldl = SQL_NTS; } else ind_ldl = SQL_NULL_DATA;
        ...
        if (r.switch_price) { std::snprintf(p_switch_price, sizeof(p_switch_price), "%s", sql_decimal_string(*r.switch_price).c_str()); ind_switch_price = SQL_NTS; } else ind_switch_price = SQL_NULL_DATA;
        if (r.switch_diff)  { std::snprintf(p_switch_diff, sizeof(p_switch_diff), "%s", sql_decimal_string(*r.switch_diff).c_str()); ind_switch_diff = SQL_NTS; } else ind_switch_diff = SQL_NULL_DATA;
    ...
    if (SQL_SUCCEEDED(rc)) {
        ...
        while (SQLFetch(verifyStmt) == SQL_SUCCESS) {
            ...
            SQLGetData(verifyStmt, 5, SQL_C_CHAR, closePrice, sizeof(closePrice), nullptr);
            ...
            std::cout << id << "  " << v_asset << "  " << seqNo << "  "
                      << tradeDate << "  " << closePrice << "  " << mov << "  " << state << "\n";
        }
    } else {
...
int main(int argc, char** argv)
{
    ...
    if (!trend_results.empty()) {
        ...
        FutureSwitch fs = find_next_switch_price(clos, last.idx, last.state);
