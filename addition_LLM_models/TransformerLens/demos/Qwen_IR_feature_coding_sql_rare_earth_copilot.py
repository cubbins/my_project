    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 429:
                print(f"  Wikipedia rate limit hit (429). Waiting {delay} seconds...")
                time.sleep(delay)
            ...
            print(f"  Wikipedia request error: {e}. Retrying in {delay} seconds...")
            time.sleep(delay)
            delay *= 1.5
    raise RuntimeError("Wikipedia request failed after multiple retries.")
...
TITLE_MAP = {
    # Normalize all variants of the China–Rare Earths WTO dispute title
    "China–Rare Earths WTO dispute": "China–Rare Earths WTO dispute",  # correct EN DASH
    "China-Rare Earths WTO dispute": "China–Rare Earths WTO dispute",  # hyphen
    "China - Rare Earths WTO dispute": "China–Rare Earths WTO dispute",  # spaced hyphen
    "China – Rare Earths WTO dispute": "China–Rare Earths WTO dispute",  # spaced en dash
    "China—Rare Earths WTO dispute": "China–Rare Earths WTO dispute",  # em dash
    "China — Rare Earths WTO dispute": "China–Rare Earths WTO dispute",  # spaced em dash

    # Other titles
    "Mountain Pass mine": "Mountain Pass mine",
    "Lynas Corporation": "Lynas Corporation",
}

TITLE_MAP.update({
    "Rare earths in Myanmar": "Mining in Myanmar",
    "Rare earths in Vietnam": "Mining in Vietnam",
    "Rare earths in India": "Mining in India",
    "Rare earths in Brazil": "Mining in Brazil",
    "Rare earths in Tanzania": "Mining in Tanzania",
    "Rare earths in Madagascar": "Mining in Madagascar",
    "Strategic mineral": "Strategic minerals",
...
def fetch_wikipedia_summary22(title: str) -> str:
    try:
        import requests
    except ImportError as exc:
...
def fetch_wikipedia_summary33(title: str) -> str:
    try:
        import requests
    except ImportError as exc:
        raise RuntimeError("requests is not installed. Install it with: pip install requests") from exc
    ...
    headers = {"User-Agent": "IR-Feature-Coding-Qwen-SQL-RareEarth/1.1"}
...
def fetch_wikipedia_summary44(title: str) -> str:
    url = "https://en.wikipedia.org/w/api.php"
    params = {
        "action": "query",
        "prop": "extracts",
    ...
    headers = {"User-Agent": "IR-Feature-Coding-Qwen-SQL-RareEarth/1.2"}

    response = safe_get(url, params, headers)
    data = response.json()
...
def fetch_wikipedia_summary(title: str) -> str:
    ...
    try:
        return _fetch_wikipedia_page(title)
    except ValueError:
        pass  # fall through to normalization

    # Second attempt: TITLE_MAP fallback
    if title in TITLE_MAP:
        mapped = TITLE_MAP[title]
        try:
            return _fetch_wikipedia_page(mapped)
        except ValueError:
            pass

    # Third attempt: Unicode dash normalization
    normalized = (
        title.replace("-", "–")   # hyphen → en dash
             .replace("—", "–")   # em dash → en dash
             .replace(" – ", "–") # spaced en dash → en dash
             .replace(" — ", "–") # spaced em dash → en dash
    )

    if normalized != title:
        try:
            return _fetch_wikipedia_page(normalized)
        except ValueError:
            pass
...
def _fetch_wikipedia_page(title: str) -> str:
    url = "https://en.wikipedia.org/w/api.php"
    params = {
        "action": "query",
        "prop": "extracts",
...
def build_feature_prompt22(event_name: str, summary: str) -> str:
    return f"""
...
def classify_supply_chain(summary: str) -> str:
    text = summary.lower()
    if "mine" in text or "mining" in text:
        return "mining"
...
You MUST extract rare-earth geopolitics if ANY of these appear:
rare earth, critical mineral, gallium, germanium, indium, lithium, cobalt,
processing, refining, export control, supply chain, dependency, leverage.
...
def run_pipeline22(config: AppConfig) -> int:
    ...
    for event_name in event_titles:
        ...
        try:
            ...
            summary = fetch_wikipedia_summary(normalize_title(event_name))
...
def run_pipeline(config: AppConfig) -> int:
    ...
    for event_name in event_titles:
        ...
        try:
            # UPDATED: normalize Wikipedia title
            summary = fetch_wikipedia_summary(normalize_title(event_name))
            ...
            # UPDATED: rare-earth supply chain classifier
            parsed["rare_earth_supply_chain_stage"] = classify_supply_chain(summary)
