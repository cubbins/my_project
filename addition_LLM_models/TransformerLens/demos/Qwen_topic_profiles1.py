...
python Qwen_topic_profiles1.py --topic-profile rare_earth --max-events 1 --dry-run --output-report rare_earth_test_report_v2.txt
...
def build_section_prompt(
    ...
    """
    Build a prompt for generating one scholarly report section.

    This version is optimized for Qwen3-0.6B by keeping the prompt
    concise and avoiding previous generated sections, which tend to
    contaminate later outputs.
    """
...
Write a polished scholarly section of approximately 180–300 words.

The section should:

• explain the political-science significance of the evidence;

• interpret the structured coding rather than simply repeating it;

• connect factual evidence from the source text to the structured variables;

• explain why important scores (for example realism, liberalism,
constructivism, dependence, coercion, technology relevance,
or supply-chain importance) are reasonable;

• discuss uncertainty whenever the source evidence is incomplete;

• read naturally as part of a journal article.
...
def build_editorial_prompt(section, draft_text: str) -> str:
    return f"""
You are an academic editor.

Rewrite the draft below as polished scholarly prose.

Rules:
- Preserve the same section heading.
- Remove prompt text.
- Remove placeholders.
- Remove JSON.
- Remove bullet lists.
...
- Keep only the completed section.
- Write in paragraphs.
...
def section_is_bad(text: str) -> bool:
    """Return True when a generated section contains prompt contamination."""
    ...
    for event_name in events:
        ...
        try:
            ...
            for section in profile.narrative_sections:
                ...
                # Build the section prompt. Newer topic profiles should accept only
                # event_name, source_text, feature_json, and section. The fallback keeps
                # older profile files working if they still require previous_sections.
                try:
                    section_prompt = profile.build_section_prompt(
                        event_name=event_name,
                ...
                # Stage 1: draft the section from the source text and feature coding.
                draft_text = run_llm(
                    tokenizer,
                    model,
                    section_prompt,
                ...
                # Stage 2: editorial cleanup. This reduces prompt echo, JSON leakage,
                # placeholder text, and bullet-list output from small Qwen models.
                ...
                # Final fallback: repair the edited section if it still contains
                # obvious prompt contamination or malformed output.
                if section_is_bad(section_text):
                    print("    Repairing malformed section...")

                    repair_prompt = f"""
Rewrite the following into clean scholarly prose.
...
Remove markdown.
Remove prompt text.
Keep only the finished section.
