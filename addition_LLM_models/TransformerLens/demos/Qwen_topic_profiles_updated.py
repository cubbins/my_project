...
Example dry run:
    ...
    python Qwen_topic_profiles_updated.py \
...
python Qwen_topic_profiles.py --topic-profile rare_earth --max-events 1 --dry-run

python Qwen_topic_profiles.py --topic-profile rare_earth --max-events 1 --dry-run --output-report rare_earth_test_report.txt
...
def parse_args(argv: Optional[list[str]] = None) -> AppConfig:
    ...
    parser.add_argument(
         "--topic-profile",
         default="rare_earth",
         choices=["rare_earth", "general_ir"],
         help="Topic profile plug-in to use."
         )
...
def load_topic_profile(profile_name: str):
    if profile_name == "rare_earth":
        from topic_profiles.rare_earth_profile import RareEarthProfile
        return RareEarthProfile()

    if profile_name == "general_ir":
        from topic_profiles.general_ir_profile import GeneralIRProfile
        return GeneralIRProfile()
...
def load_event_titles22(event_file: Optional[str]) -> list[str]:
    if event_file:
...
def load_event_titles(event_file: Optional[str], default_events: list[str]) -> list[str]:
    if event_file:
        path = Path(event_file).expanduser()
        with path.open("r", encoding="utf-8") as f:
            titles = [
...
def run_smoke_test(tokenizer, model) -> None:
    ...
    # Also, your smoke test output shows Qwen is over-answering. 
    # That is not fatal, but you can improve it by changing:
    output = run_llm(tokenizer, model, prompt, max_new_tokens=10)
...
#   Therefore select_relevant_text() should receive profile: 
...
def select_relevant_text(
    ...
    # cubbins
    terms = profile.build_relevance_terms(event_name)
...
def build_section_prompt(
    ...
    """
    Build one section prompt.

    The previous_sections argument is accepted for backward compatibility,
    but it is intentionally ignored for Qwen3-0.6B. Passing prior generated
    sections caused prompt echoing and template contamination in test reports.
    """

    return f"""
You are an expert political scientist writing for researchers, graduate students,
and advanced undergraduate students studying International Relations.

Write ONE completed report section.

Use ONLY the information contained in:
1. the structured feature coding
2. the supplied source text
...
Do NOT write "Answer:".
Do NOT write "[feature_name]".
Do NOT write "[value]".
Do NOT write "[Your answer here]".
Do NOT repeat the structured feature coding.
Do NOT repeat the source text verbatim.
...
Task:
Write a polished scholarly section of approximately 180 to 300 words.

The section should interpret the structured coding, connect factual evidence
from the source text to the variables, explain the political-science logic of
important scores, and state uncertainty where the evidence is incomplete.

Output requirements:
The first line MUST be exactly:
{section['number']}. {section['title']}

After the heading, write only continuous scholarly prose.
Do not output anything else.
""".strip()


def build_section_prompt_from_profile(
    profile,
    event_name: str,
    source_text: str,
    feature_json: dict[str, Any],
    ...
    """
    Build a section prompt from the active topic profile while preventing
    previous-section contamination. Supports both old and new profile method
    signatures.
    """
    try:
        return profile.build_section_prompt(
...
def section_is_bad(text: str) -> bool:
    """Return True when a generated section contains prompt echo or template debris."""
...
def build_section_repair_prompt(section: dict[str, Any], bad_draft: str) -> str:
    """
    Build a conservative repair prompt. It is used only when validation fails,
    not as a mandatory second pass. This preserves academic depth when the
    first draft is already acceptable.
    """
    return f"""
You are an academic editor.

Rewrite the draft below as a clean scholarly International Relations section.

Remove placeholders, JSON, markdown, bullet lists, code fences, prompt text,
and repeated instructions. Do not add new facts. Preserve the academic meaning.
...
def feature_json_is_weak(parsed: Optional[dict[str, Any]]) -> bool:
    """Detect parsed feature JSON that is structurally present but substantively failed."""
    if parsed is None:
        return True

    if parsed.get("quality_flag") in {"PLACEHOLDER", "FAILED"}:
        return True
    ...
    required_nonempty = [
        ...
        "conflict_type",
        "escalation_stage",
        "resource_leverage_type",
        "supply_chain_stage",
    ]
    if any(parsed.get(name) in {None, "", "unclear"} for name in required_nonempty):
        return True

    score_fields = [
        "import_dependence_score",
        "export_control_intensity",
        "china_centrality_score",
        "diversification_pressure_score",
        "realism_score",
        "liberalism_score",
        "constructivism_score",
    ]
    null_scores = sum(parsed.get(name) is None for name in score_fields)
    return null_scores >= 3
...
def print_header(config: AppConfig, device: str, dtype: torch.dtype) -> None:
    print("=" * 100)
    print(f"Qwen Multi-Stage Narrative Runner | profile={config.topic_profile}")
    print("=" * 100)
...
The case of {event_name} can be interpreted as an International Relations case involving {parsed.get('conflict_type', 'unclear')} under conditions of {parsed.get('polarity', 'unclear')} politics. The structured coding identifies the escalation stage as {parsed.get('escalation_stage', 'unclear')} and the central resource-leverage mechanism as {parsed.get('resource_leverage_type', 'unclear')}. These classifications suggest that the case is not primarily a military confrontation, but a dispute shaped by economic pressure, institutional bargaining, and control over strategically important supply-chain stages.

The evidence indicates that economic and diplomatic instruments are more important than military or informational coercion. The coding assigns China centrality a score of {parsed.get('china_centrality_score', 'unclear')}, export-control intensity a score of {parsed.get('export_control_intensity', 'unclear')}, and diversification pressure a score of {parsed.get('diversification_pressure_score', 'unclear')}. These values imply that the case should be understood through the politics of dependence, vulnerability, and supply-chain adjustment.

The theoretical scores also clarify the interpretation. A realism score of {parsed.get('realism_score', 'unclear')} emphasizes strategic competition and resource power. A liberalism score of {parsed.get('liberalism_score', 'unclear')} points to the relevance of trade rules and institutional dispute settlement. A constructivism score of {parsed.get('constructivism_score', 'unclear')} suggests that norms and identities are less central than material and institutional factors. The main evidentiary basis is: {parsed.get('evidence_text', 'No specific evidence text was available.')}
""".strip()
...
def run_pipeline(config: AppConfig) -> int:
    ...
    profile = load_topic_profile(config.topic_profile)
    ...
    if config.output_report:
        config.output_report.write_text(
            f"{profile.theme.upper()} MULTI-STAGE FEATURE + NARRATIVE REPORT\n"
            + "=" * 120
            + "\n"
    ...
    events = load_event_titles(config.event_file, profile.default_events)
    ...
    for event_name in events:
        ...
        try:
            ...
            feature_prompt = profile.build_feature_prompt(event_name, selected_text)
            ...
            if feature_json_is_weak(parsed):
                raise RuntimeError(
                    "Qwen feature output parsed, but the structured coding is weak "
                    "or placeholder-like. Re-run or adjust the feature prompt before "
            ...
            for section in profile.narrative_sections:
                ...
                section_prompt = build_section_prompt_from_profile(
                    profile=profile,
                    event_name=event_name,
                ...
                if section_is_bad(section_text):
                    print("    Bad section detected; using deterministic fallback section.")

                    section_text = build_fallback_section(
                        event_name=event_name,
                        section=section,
                        parsed=parsed,
                    )
