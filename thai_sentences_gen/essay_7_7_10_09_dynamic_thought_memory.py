...
class IntermediateInstructionMemory:
    ...
    - It stores system prompt, user prompt, model output, and metadata.
    - It creates a compact memory text from recent prior outputs.
    - That compact memory text is prepended to the next LLM call.
    """
    ...
    def build_memory_string(self) -> str:
        ...
        for i, item in enumerate(items, start=1):
            ...
            thought_seconds = item.get("thought_seconds", 0.0)
            thought_weight = item.get("thought_weight", 1.0)
            ...
            blocks.append(
                ...
                f"thought_seconds={thought_seconds:.3f}; weight={thought_weight:.3f}]\n"
                f"{output}\n"
            )
...
class MemoryAugmentedLLMRouter:
    ...
    def call(self, system_prompt: str, user_prompt: str, **generate_kwargs) -> str:
        ...
        memory_text = self.memory.build_memory_string()
...
"""
        ...
        self.memory.save_call(
            ...
            prompt_chars=len(system_prompt) + len(augmented_user_prompt),
