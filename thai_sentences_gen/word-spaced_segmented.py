...
def segment_thai_sentence(sentence: str) -> str:
    """
    Return a word-spaced Thai sentence for readability.

    Thai is normally written without spaces between words.
    This function creates a learning/inspection version.

    Requires:
        pip install pythainlp

    If PyThaiNLP is not installed, the function returns the original sentence.
    """
...
"""
    ...
    thai_segmented = segment_thai_sentence(thai_sentence)

    return {
        "level": str(data["level"]).strip(),
        "used_words": data["used_words"],
        "thai_sentence": thai_sentence,
        "thai_segmented": thai_segmented,
...
def write_report(words, results, filename: str = OUTPUT_FILE):
    with open(filename, "w", encoding="utf-8") as f:
        ...
        for index, item in enumerate(results, start=1):
            ...
            f.write("THAI Word-Spaced:\n")
            f.write(f"{item['thai_segmented']}\n\n")
